class Teacher::AbsencesController < ApplicationController
  ...
  def update
    student = current_teacher.school_class_students.find(params[:absence][:student_id])
    absence = student.absences.find_by_date_and_semester_id(params[:absence][:date], current_teacher.school_class_semester.id)
    if absence
